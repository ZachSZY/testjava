...
public class TennisScore {
    public static String GetTennisScore(int server_score, int receiver_score) {
        ...
        Map<Integer,String> ahead_side = new HashMap<>();
        ahead_side.put(-1,"Receiver");
        ahead_side.put(-2,"Receiver");
        ahead_side.put(1,"Server");
        ahead_side.put(2,"Server");
        String result = "";
        if (server_score < 3 && receiver_score < 3) {
            ...
            if (server_score == receiver_score) {
                ...
                result = ahead_side.get((server_score - receiver_score))+" "+condition_array[2];
                return result;
            }else if(abs(server_score - receiver_score) == 2){
                result = ahead_side.get((server_score - receiver_score))+" "+condition_array[3];
                return result;
            }
        }
        return null;
